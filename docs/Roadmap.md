...
- **Windows Direct2D 后端**：`Square.Backends.Direct2D` 使用 DirectNAot 和 `ID2D1HwndRenderTarget` 直接绘制 Win32 HWND，覆盖几何、渐变、stroke、矩形/几何 clip、group opacity、bitmap 版本刷新和现有 glyph coverage 文本；首版使用全帧渲染，设备目标丢失后由宿主请求 DisplayTree 重放，renderer 截图暂时回退 Software。真实窗口像素测试覆盖 resize、clip/layer、图片和文本，D3D11/DXGI readback、dirty present 与 DirectWrite 属于后续阶段。
