...
截图请求在 UI 线程排队，并在布局、绘制及后端 `Present` 完成后返回位图；待处理截图本身计入帧工作，空闲窗口也会处理一帧，避免读取首帧或调整窗口尺寸后尚未绘制的缓冲区，无需固定延时或重试。外部 `ApplicationSession` 宿主必须在等待截图期间继续驱动帧处理；会话分离时尚未完成的截图请求以异常结束。

截图请求从 API 调用时起受会话生命周期管理，包括尚未由 Dispatcher 处理的请求。会话分离后再次请求截图会立即返回以 `InvalidOperationException` 失败的 Task，无需继续运行事件循环。渲染器创建、布局、绘制或后端提交失败时，待处理截图收到原始异常，渲染调用也继续抛出该异常；仍保持附加的会话恢复正常渲染后，可以重新请求截图。
