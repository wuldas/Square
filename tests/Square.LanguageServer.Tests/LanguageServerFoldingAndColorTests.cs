...
public sealed class LanguageServerFoldingAndColorTests
{
    [Fact]
    public async Task FoldingAndDocumentColorUseSyntaxTreeAndCssValues()
    {
        ...
        await Write(process, """{"jsonrpc":"2.0","id":2,"method":"textDocument/foldingRange","params":{"textDocument":{"uri":"file:///C:/Square/Fold.sqx"}}}""");
        var folding = await Read(process.StandardOutput);
        Assert.Contains("\"startLine\":0", folding, StringComparison.Ordinal);
        Assert.Contains("\"endLine\":4", folding, StringComparison.Ordinal);
        Assert.Contains("\"startLine\":1", folding, StringComparison.Ordinal);

        await Write(process, """{"jsonrpc":"2.0","id":3,"method":"textDocument/documentColor","params":{"textDocument":{"uri":"file:///C:/Square/Fold.sqx"}}}""");
        var colors = await Read(process.StandardOutput);
        Assert.Contains("\"red\":", colors, StringComparison.Ordinal);
        Assert.Contains("\"line\":6", colors, StringComparison.Ordinal);
