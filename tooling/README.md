...
This phase includes file recognition, TextMate highlighting, embedded C#/CSS scopes, brackets, folding, and snippets where supported by the host. The shared Language Server adds diagnostics plus syntax-tree template/CSS/C# script completion, hover, symbols, definition navigation, and semantic tokens. Completion items carry markdown documentation naming the target type, its owning component, and the source file. It declares UTF-16 position encoding, negotiates hover markdown and definition link support from client capabilities, registers file watchers for template and project files, and honours `$/cancelRequest`.
