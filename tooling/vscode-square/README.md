...
When `square.languageServer.path` is empty, the extension starts the bundled server DLL. Completion uses the shared Square syntax tree: tags, control-flow directives, attributes, events, Vue directives, and CSS class names. Inside `<style>`, completion covers Square CSS properties, property-specific values, built-in control selectors, template class/ID selectors, pseudo selectors, custom properties, and supported at-rules. Inside `<script>`, completion covers C# keywords and types, using namespaces, Square/C# attributes and named arguments, current script members, method locals/parameters, template refs, and common Square/control members. Parsed section/CSS/Roslyn trees are reused across diagnostics, completion, hover, symbols, folding, color, and semantic-token requests, with bounded eviction and close-time invalidation. Hover, folding, CSS color decorations, document symbols, definition navigation, and semantic tokens are also provided.
